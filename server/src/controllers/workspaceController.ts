...
export const getAllWorkspaces = async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
        const response = await workspaceService.getAllWorkspaces(req.user.userId);
        return successResponse(res, { data: response });
    } catch (error) { next(error); }
};
...
export const deleteWorkspace = async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
        const response = await workspaceService.deleteWorkspace(String(req.params.id), req.user.userId);
        return successResponse(res, { data: response });
    } catch (error) { next(error); }
};
